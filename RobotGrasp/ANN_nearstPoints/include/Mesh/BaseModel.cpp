...
pair<vector<double>, double> CBaseModel::GetVoronoiAreas() const
{
	vector<double> voronoiAreas(GetNumOfVerts(), 0);
	double totalArea(0);
	for (int i = 0; i < GetNumOfFaces(); ++i)
	{
		...
		normal.Normalize();
		totalArea += area;
		if (((pt2 - pt1) ^ (pt3 - pt1)) < 0)
		{
			voronoiAreas[Face(i)[0]] += area / 2;
			voronoiAreas[Face(i)[1]] += area / 4;
			voronoiAreas[Face(i)[2]] += area / 4;
		}
		else if (((pt3 - pt2) ^ (pt1 - pt2)) < 0)
		{
			voronoiAreas[Face(i)[1]] += area / 2;
			voronoiAreas[Face(i)[0]] += area / 4;
			voronoiAreas[Face(i)[2]] += area / 4;
		}
		else if (((pt1 - pt3) ^ (pt2 - pt3)) < 0)
		{
			voronoiAreas[Face(i)[2]] += area / 2;
			voronoiAreas[Face(i)[0]] += area / 4;
			voronoiAreas[Face(i)[1]] += area / 4;
		}
		else
		{
			...
			M << 2 * (pt1.x - pt2.x), 2 * (pt1.y - pt2.y), 2 * (pt1.z - pt2.z),
				2 * (pt1.x - pt3.x), 2 * (pt1.y - pt3.y), 2 * (pt1.z - pt3.z),
				2 * (pt2.x - pt3.x), 2 * (pt2.y - pt3.y), 2 * (pt2.z - pt3.z),
				normal.x, normal.y, normal.z;
			right << (pt1 ^ pt1) - (pt2 ^ pt2),
				(pt1 ^ pt1) - (pt3 ^ pt3),
				(pt2 ^ pt2) - (pt3 ^ pt3),
				(normal ^ pt1);
			Eigen::VectorXd result = (M.transpose() * M).inverse() * M.transpose() * right;
			CPoint3D center(result(0), result(1), result(2));
			double area12 = VectorCross(pt1, pt2, center).Len();
			double area23 = VectorCross(pt2, pt3, center).Len();
			double area13 = VectorCross(pt1, pt3, center).Len();
			voronoiAreas[Face(i)[0]] += (area12 + area13) / 2;
			voronoiAreas[Face(i)[1]] += (area12 + area23) / 2;
			voronoiAreas[Face(i)[2]] += (area13 + area23) / 2;
		}
	}
	return make_pair(voronoiAreas, totalArea);
}